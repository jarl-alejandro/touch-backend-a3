...
export default class TouchBackend {
	...
	constructor(manager, context, options) {
		...
		this.handleTopMoveStart = (e) => {
			...
			console.log({
				managerTouch: this.managerTouch,
				contextTouch: this.contextTouch,
				optionsTouch: this.optionsTouch,
			})

			if (!shouldIgnoreTarget(e.target)) {
				const $el = closest(e.target, '.TreeView-box')
			...
			if (clientOffset) {
				console.log({ isTouchEvent: isTouchEvent(e) })
		...
		this.handleTopMove = (e) => {
			...
			const dragging = document.querySelector('.is-draggin-a3')
